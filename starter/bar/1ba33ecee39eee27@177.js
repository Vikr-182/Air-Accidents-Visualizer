...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([["subs_batsmen.csv",new URL("./files/c07d0f21ac305c674a17a297e9839863c4179f10d0c340fc12bcbc44fe9aafd2a487a4f2231298325ce34f3035d60978f87a280353bbb5a934dc931fd641ee8d",import.meta.url)]]);
  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
  main.variable(observer()).define(["md"], function(md){return(
md`# batsmen ranking Bar Chart Race
`
)});
...
require("d3@5", "d3-array@2","d3-time")
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`A subset of the given data has been taken since the original file was extremely large and was crashing my laptop, in order to run for the orginal file all you need to do is change the name of the attachment to <b>batsmen_ratings_all091217.csv<b>`
)});
  main.variable(observer("data1")).define("data1", ["d3","FileAttachment"], async function(d3,FileAttachment){return(
d3.csvParse(await FileAttachment("subs_batsmen.csv").text(), d3.autoType)
)});
  main.variable(observer("data")).define("data", ["data1"], function(data1)
{
  data1.forEach(function(d){
  d.name = d.name +" (" +d.nations + ")";
})
  return data1
}
);
